...
pip install pillow clu tensorflow==2.15.0 'keras<3' 'torch<=2.4' torchvision tensorflow_datasets matplotlib==3.9.2 > /dev/null
...
# on some worker, this will fail, run again
# check if torch is installed
if ! python -c "import torch" &> /dev/null; then
    pip install 'torch<=2.4' clu tensorflow==2.15.0 'keras<3' torchvision tensorflow_datasets matplotlib==3.9.2
fi
